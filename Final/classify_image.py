...
#serial communication and camera inits
import serial
import os
...
def id_string(string):
   for i in range(0,len(string)):
	if ( string[i] == 'c' and (i + 1) != '/0'):
	    if ( string[i + 1] == 'a' and string[i + 2] != '/0'):
		if ( string[i + 2] == 't' ):
		   cat_found = 1
                   break    #found cat
            else:
...
def run_inference_on_image(image):
  ...
  with tf.Session() as sess:
    ...
    for node_id in top_k:
      ...
      cat = id_string(str(human_string))
      if ( cat ):
        send_data = '1'
	print('Found Cat: ' + send_data)
	break
      else:
        send_data = '0'
	print('No Cat: ' + send_data)
        break
...
def main(_):
# called initially to download tensorflow files
